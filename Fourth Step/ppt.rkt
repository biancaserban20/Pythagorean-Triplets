...
(define ppt-stream-in-tree-order
  (let iter ((q (stream '(3 4 5)))) 
    (let ((s1 (multiply T1 (stream-first q)))
          (s2 (multiply T2 (stream-first q)))
          (s3 (multiply T3 (stream-first q)))
          )
      (stream-cons (stream-first q) (iter (stream-tail (stream-append q (stream s1 s2 s3)) 1)))))) 
...
(define (pairs G H)
  (let pairing ( (ig 0) (ih 1) (g G) (h H) )
    (if (equal? ig ih)
        (pairing 0 (add1 ih) G (stream-tail h 1))
        (stream-cons (cons (stream-first g) (stream-first h)) (pairing (add1 ig) ih (stream-tail g 1) h)))))
...
(define (naturals-from n)
  (let loop ((seed n))
    (stream-cons seed (loop (+ seed 2)))))

(define (prim x)
  (if (equal? (gcd (car x) (cdr x)) 1)
      #t
      #f))

(define gh-pairs-stream
  (let* ((G (naturals-from 1)) (H (naturals-from 3)) (s (pairs G H)))
    ( stream-filter prim s)))
...
(define ppt-stream-in-pair-order
  (let TPP ((s gh-pairs-stream))
    (let* ( (g (car (stream-first s)))
            (h (cdr (stream-first s)))
            (e (/ (- h g) 2))
            (f (/ (+ h g) 2))
            (a (* g h))
            (b (* 2 e f))
      ...
      (stream-cons (list a b c) (TPP (stream-tail s 1))))))
