...
(define (mul-aux M V rez)
  (if (null? M) (reverse rez)
      (mul-aux (cdr M) V (cons (dot-product (car M) V) rez))))
...
(define (matrix-aux Ts ppt)
  (if (null? Ts) ppt
      (cond ((equal? (car Ts) 1) (matrix-aux (cdr Ts) (multiply T1 ppt)))
            ((equal? (car Ts) 2) (matrix-aux (cdr Ts) (multiply T2 ppt)))
            (else (matrix-aux (cdr Ts) (multiply T3 ppt))))))
