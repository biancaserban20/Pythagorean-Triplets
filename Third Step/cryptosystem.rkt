...
(define (key n)
  (let* ((n (get-nth-quadruple n)) (e (cadr n)) (f (caddr n))
        (a1 (a1 e f)) (b1 (b1 e f)) (c1 (c1 e f))
        (a2 (a2 e f)) (b2 (b2 e f)) (c2 (c2 e f))
        (a3 (a3 e f)) (b3 (b3 e f)) (c3 (c3 e f))
        (L (list a1 b1 c1 a2 b2 c2 a3 b3 c3)))
...
(define (message->codes message)
  (let* ((lst (string->list message))
        (msg (map char->integer lst)))
    (map (λ(x) (if (equal? x 32)
                   0
                   (- x 96)
                   )) msg)))
...
(define (codes->message codes)
  (let* ((list (map (λ(x)(if (equal? x 0)
                             32
                             (+ x 96))) codes))
          (rcodes (map integer->char list)))
    (list->string rcodes)))
...
(define (extend-key key size)
  (let ((len (length key)))
    (cond ((= size len) key)
          ((> len size) (reverse (list-tail (reverse key) (- len size))))
          (else
           (let iter((k key) (l len) (lst key))
             (cond ((= l size) k)
                   ((null? lst) (iter k l (append lst key)))
                   (else (iter (append k (list (car lst))) (+ l 1) (cdr lst))))
             )))))
